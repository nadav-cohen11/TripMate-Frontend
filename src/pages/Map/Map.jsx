  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        toast.error(extractBackendError(err) || "Failed to fetch user locations.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-blue-50 to-blue-200 flex flex-col items-start py-8 px-4 space-y-8 max-w-6xl mx-auto w-full">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center w-full h-[60vh] text-gray-800 text-center">
          <h1 className="text-2xl font-semibold">No users to display on the map right now!</h1>
    ...
    >
      ...
      {userLocations.map(renderUserMarker)}
        ...
        <div className="w-full max-w-6xl mt-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4">🌍 Featured NeatBy ME</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {userLocations.slice(0, 10).map((user, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md p-4 flex flex-col items-center hover:shadow-lg transition">
                <img
                ...
                />
                <p className="text-center font-medium text-gray-700">{user.fullName || "Unnamed User"}</p>
              </div>
            ))}
          </div>
        </div>
